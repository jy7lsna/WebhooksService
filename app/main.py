...
=======
        "event_types": db_sub.event_types.split(",") if db_sub.event_types else []
    })
    return db_sub
...
async def update_subscription(subscription_id: int, sub: schemas.SubscriptionUpdate, db: Session = Depends(get_db)):
    ...
    if not updated:
        raise HTTPException(status_code=404, detail="subscriptions not found")
=======
async def update_subscription(subscription_id: str, sub: schemas.SubscriptionUpdate, db: Session = Depends(get_db)):
    updated = crud.update_subscription(db, subscription_id, sub)
    if not updated:
        raise HTTPException(status_code=404, detail="Subscription not found")
>>>>>>> 65cd0d2 (Initial Commit)
...
=======
        "event_types": updated.event_types.split(",") if updated.event_types else []
    })
    return updated
...
=======
    ...
    if sub.event_types:
        allowed_events = sub.event_types.split(",")
        if event_type and event_type not in allowed_events:
            return JSONResponse(status_code=204, content={"detail": "Event type not subscribed"})

    # Queue delivery task asynchronously
    tasks.deliver_webhook.delay(subscription_id, payload, 1)
...
def get_subscription_logs(subscription_id: int, db: Session = Depends(get_db)):
=======
def get_subscription_logs(subscription_id: str, db: Session = Depends(get_db)):
>>>>>>> 65cd0d2 (Initial Commit)
    logs = crud.get_delivery_logs_by_subscription(db, subscription_id)
    return logs
